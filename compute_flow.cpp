...
int main( int argc, char *argv[] )
{
	...
	int type = 1;
	int frameSkip = 1;
	std::string vid_path = "Videos/";
	std::string out_path = "out_path/";
	std::string out_path_jpeg = "out_path_jpeg/";
	bool is_out_jpeg = 1;
	float MIN_SZ = 256;
	float OUT_SZ = 256;
	...
	const char* keys = "{ h  | help      | false | print help message }"
				...
				"{ s  | skip     |  1    | frame skip}"
				"{ d  | vid_path     |  /media/peng/Store/UCF_Crimes/Videos/  |  string    | video path}"
				"{ o  | out_path     |  /media/peng/BigData/gpu_flow/out_path/  |  string    | output path}"
				"{ j  | out_path_jpeg     |  /media/peng/BigData/gpu_flow/out_path_jpeg/  |  string    | output jpeg path}"
				"{ i  | is_out_jpeg     |  1    |  bool | is out jpeg}" 
				"{ m  | MIN_SZ     |  256    |  float  | minimize size}"
				"{ u  | OUT_SZ     |  256    |  float  | minimize output size}"
				"{ c  | clipFlow     |  true    |  bool  | clip flow}"
				"{ r  | resize_img     |  1    |  bool  | resize img}";
	...
	if (argc > 1) {
		...
		is_out_jpeg = cmd.get<bool>("is_out_jpeg");
		MIN_SZ = cmd.get<float>("MIN_SZ");
		OUT_SZ = cmd.get<float>("OUT_SZ");
		clipFlow = cmd.get<bool>("clipFlow");
		resize_img = cmd.get<bool>("resize_img");
		vid_path = cmd.get<string>("vid_path");
		out_path = cmd.get<string>("out_path");
		out_path_jpeg = cmd.get<string>("out_path_jpeg");

		cout << "start_vid:" << start_with_vid << " gpuID:" << gpuID << " flow method: " << type << " frameSkip: " << frameSkip << endl << " vid_path: " << vid_path << endl << " out_path: " << out_path << endl <<" out_path_jpeg: " << out_path_jpeg << endl << " is_out_jpeg: " << is_out_jpeg << " resize_img: "<<resize_img<<" clipFlow: "<<clipFlow<<" MIN_SZ: "<<MIN_SZ<<" OUT_SZ: "<<OUT_SZ<<endl;
	}
	...
	for (; (dirIt.hasNext()); )         
	{
		...
		QString file = dirIt.fileName();
			...
			std::string outfile = out_path + "u/" + fName + ".txt";

			FILE *fx = fopen(outfile.c_str(),"w");
